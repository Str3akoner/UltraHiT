...
from datasets import ACTION_KEYS, SeqDataset_Adaptive_Corrector
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a sequential multi-class adaptive corrector")

    parser.add_argument("--train-csv", "--data_source", dest="train_csv", required=True, type=str, help="Path to training CSV")
    parser.add_argument("--val-csv", "--eval_data_path", dest="val_csv", required=True, type=str, help="Path to validation CSV")
    ...
    parser.add_argument("-b", "--batch-size", default=256, type=int, help="Global batch size")
    parser.add_argument("--lr", "--learning-rate", dest="lr", default=1e-4, type=float, help="Learning rate")
    parser.add_argument("--weight-decay", "--wd", dest="weight_decay", default=1e-3, type=float, help="Weight decay")
    parser.add_argument("--momentum", default=0.9, type=float, help="Momentum for SGD")
    parser.add_argument("--optimizer", default="adam", choices=["adam", "sgd"], help="Optimizer")
    parser.add_argument("--scheduler", default="cosine", choices=["step", "cosine"], help="LR scheduler")
    ...
    parser.add_argument("--pretrain", default="", type=str, help="Pretrained checkpoint path")
    parser.add_argument("--disable-backbone-pretrain", action="store_true", help="Disable backbone pretraining")
...
def compute_class_weights(counts: np.ndarray, strategy: str, beta: float = 0.99) -> np.ndarray:
    ...
    if counts.ndim != 1:
        raise ValueError(f"Expected 1D class counts, but got shape {counts.shape}")
    if counts.sum() <= 0:
    ...
    if strategy == "weighted":
        freqs = counts[valid] / counts[valid].sum()
        weights[valid] = (1.0 / freqs) / len(counts)
    elif strategy == "sqrt-weighted":
        freqs = counts[valid] / counts[valid].sum()
        weights[valid] = np.sqrt((1.0 / freqs) / len(counts))
...
def build_optimizer(args, model: nn.Module):
    if args.pretrain:
        ...
        for name, param in model.named_parameters():
            if name.startswith("actors.") or name.startswith("distancers."):
        ...
        param_groups = []
        if backbone_params:
            param_groups.append({"params": backbone_params, "lr": args.lr * 0.5})
        if head_params:
...
def validate_multi_hit(
    ...
    for img_seq, past_actions, target_mask in val_loader:
        img_seq = img_seq.to(device, non_blocking=True)
        past_actions = past_actions.to(device, non_blocking=True)
        ...
        hits = target_mask.gather(1, preds.view(-1, 1)).squeeze(1)
        total_correct += hits.sum()
        total_samples += float(preds.size(0))

    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(total_correct, op=dist.ReduceOp.SUM)
        dist.all_reduce(total_samples, op=dist.ReduceOp.SUM)

    acc = (total_correct / (total_samples + 1e-9)).item() * 100.0

    if is_main_process():
        logger.info(f"[VAL] Epoch {epoch}: Acc@1 (multi-hit) = {acc:.2f}%")
        if writer is not None:
            writer.add_scalar("Acc1_multi", acc, epoch)
...
def main():
    ...
    train_dataset = SeqDataset_Adaptive_Corrector(
    ...
    val_dataset = SeqDataset_Adaptive_Corrector(
    ...
    start_epoch, best_val_acc = load_resume_checkpoint(args, raw_model, optimizer, scheduler, logger)
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        if is_main_process():
            ...
            if val_stats is not None and val_stats["acc_multi"] > best_val_acc:
                best_val_acc = val_stats["acc_multi"]
                state["best_val_acc"] = best_val_acc
                save_checkpoint(state, log_dir / "checkpoint_best.pth.tar")
                logger.info(f"Saved new best checkpoint at epoch {epoch} with val acc = {best_val_acc:.2f}%")
